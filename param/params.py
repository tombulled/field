...
from .resolver import Resolver, MutableResolvers, Resolvers
...
class ParamsI(Protocol):
    ...
    def resolve_context(self, context: ResolutionContext[M], argument: Any) -> Any:
        ...
...
class NewParams:
    ...
    def resolve_context(self, context: ResolutionContext[M], argument: Any) -> Any:
        resolver: Optional[Resolver[M, Any]] = self.get_resolver(context.metadata)

        if resolver is None:
            raise ResolutionError(
                f"No resolver available for metadata of type {type(context.metadata)!r}"
            )

        return resolver(context, argument)
